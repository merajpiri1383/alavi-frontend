...
const ListForm: FC<ListFormProps> = ({
  ...
  // <-- fixed: use if/else instead of ternary used as a standalone expression
  const submitHandler = () => {
    if (currentAskForm) {
      setStage(2);
    } else {
      toast.error(<ToastError message="لطفا یکی از فرم ها را انتخاب کنید" />);
    }
  };
      ...
      <div className="flex items-center justify-between mt-12 md:my-0 mx-[30px]">
        <div>
          ...
          <p className="text-[#5A6166] font-[500] text-[12px] md:text-[14px]">
            مشتری عزیز یکی از فرم های زیر انتخاب نمایید.
          </p>
        </div>

        <div
          className="size-[14px] cursor-pointer active:scale-110"
      ...
      <div className="grid grid-cols-1 items-between h-full">
        <div className="mx-[30px] mt-8 col-span-1 md:ml-24">
          {askFormTypeList.map((item, index) => {
            return (
              <div
                className="flex items-center justify-start my-4 gap-[10px] cursor-pointer"
                key={index}
                onClick={() => setAskForm(item.type)}
              >
                <div
                  className={`size-[22px] rounded-[4px] transition md:size-[30px]
                ...
                <p className="text-[#5A6166] font-[500] text-[14px] md:text-[16px]">
                  {item.text}
                </p>
              </div>
            );
        ...
        <div
          className="mx-[30px] bg-[#2F2F2F] p-[14px] rounded-[10px] cursor-pointer col-span-1
                    h-fit md:w-fit md:px-6 md:mt-6"
          onClick={submitHandler}
        >
          <p className="text-[#FFFFFF] font-[500] text-[14px] text-center">
            ثبت و ادامه فرآیند
          </p>
        </div>
      </div>
